...
rule check8am(uint256 t1, uint256 t2) {
  require(t1 > t2);
  require (oneWeek() == dateOffset() );
  uint256 x = get8amWeeklyOrDailyAligned(t1);
  uint256 y = get8amWeeklyOrDailyAligned(t2);
  mathint diff = (x - y) / dateOffset() ;
  assert ( x >= y);
  assert (x - y == diff * dateOffset());
}
...
rule price_space(address u, address c, uint256 t1, uint256 t2, uint256 t, method f){
    ...
    assert( _price(u, c, t) == 0, "Hey, I'm here" );
}
...
rule verify_setSettlementPriceForDate() {
  ...
  uint256 p_post = _price(u, c, t);
  uint256 price = getCurrentPrice(e, u, c);
  uint256 t_alligned = get8amWeeklyOrDailyAligned(t);
