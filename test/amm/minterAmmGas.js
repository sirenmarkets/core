...
contract("Minter AMM Gas Measurement", (accounts) => {
  ...
  it("Measures gas for single-market", async () => {
    await measureGas(1)
  })

  it("Measures gas for 2 markets", async () => {
    await measureGas(2)
  })

  it("Measures gas for 4 markets", async () => {
    await measureGas(4)
  })

  it("Measures gas for 8 markets", async () => {
    await measureGas(8)
  })

  measureGas = async (numMarkets) => {
    console.log("**************************************************")
    console.log(`Measuring gas for AMM with ${numMarkets} markets:`)
    console.log("**************************************************")
    ...
    for (let i = 0; i < numMarkets; i++) {
      ...
      assert.isBelow(
        ret.receipt.gasUsed,
        320000, // TODO: this is too high, we should aim for <= 200K
      ...
      assert.isBelow(
        ret.receipt.gasUsed,
        250000, // TODO: this is too high, we should aim for <= 200K
        "bTokenBuy gas should be below threshold",
      )
      ...
      assert.isBelow(
        ret.receipt.gasUsed,
        300000, // TODO: this is too high, we should aim for <= 200K
    ...
    assert.isBelow(
      ret.receipt.gasUsed,
      1200000, // TODO: try bringing this lower
    ...
    assert.isBelow(
      ret.receipt.gasUsed,
      1000000, // TODO: too high for 8 contracts
      "LP deposit gas should be below threshold",
    )

    // Move the block time into the future so some contracts are expired
    await time.increase(oneDay * 50)
