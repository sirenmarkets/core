...
contract("wToken Vault", (accounts) => {
  ...
  it("Single LP", async () => {
    ...
    const ret2 = await deployedAmm.bTokenSell(seriesId, 1e8, 0, {
      from: aliceAccount,
    })
    let lockedCollateral = Math.floor(
      (1e8 - ret2.logs[0].args["collateralPaid"]) / 2,
    )
    assertBNEq(
    ...
    const ret3 = await deployedAmm.bTokenSell(seriesId2, 2e8, 0, {
      from: aliceAccount,
    })
    lockedCollateral += Math.floor(
      (2e8 - ret3.logs[0].args["collateralPaid"]) / 2,
    )
    ...
    await checkBalances(
      ...
      wTokenIndex,
      lpToken,
      1012000542, // 10.12e8 LP made 0.12e8 profit
      0,
      0,
      0,
    )
  ...
  it.only("Multiple LPs", async () => {
    ...
    ret = await deployedAmm.withdrawLockedCollateral(1, { from: ownerAccount })
    parseLogs(ret, deployedWTokenVault.contract) // parse WTokenVault events
    ...
    expectEvent(ret, "LpSharesRedeemed", {
      ...
      expirationId: "1",
      numShares: "13092381",
      collateralAmount: "13264013", // 1.013 per share
    })
    ...
    expectEvent(ret, "LpSharesRedeemed", {
      ...
      expirationId: "1",
      numShares: "24490217",
      collateralAmount: "24811269", // 1.013 per share
    })
    ...
    ret = await deployedAmm.withdrawCapital(2e8, false, 0, { from: lp3Account })
    parseLogs(ret, deployedWTokenVault.contract) // parse WTokenVault events
    ...
    ret = await deployedAmm.withdrawLockedCollateral(2, { from: ownerAccount })
    parseLogs(ret, deployedWTokenVault.contract) // parse WTokenVault events
    ...
    expectEvent(ret, "LpSharesRedeemed", {
      ...
      expirationId: "2",
      numShares: "11810386",
      collateralAmount: "11482247", // 0.97 per share
    })
    ...
    assertBNEq(
      await deployedAmm.lockedCollateral(),
      0,
      `AMM locked collateral should be 0`,
    )
    ...
    for (const exp of ["1", "2"]) {
      assertBNEq(
        await deployedWTokenVault.lpSharesSupply(deployedAmm.address, exp),
        0,
        `LP shares supply should be 0`,
      )

      for (const lpAccount of [ownerAccount, lp2Account, lp3Account]) {
        assertBNEq(
          await deployedWTokenVault.lpShares(
            deployedAmm.address,
            exp,
            lpAccount,
          ),
          0,
          `LP shares should be 0`,
        )
      }
    }
  })

  it("Works for puts", async () => {
