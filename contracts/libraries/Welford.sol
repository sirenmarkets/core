...
library Welford {
    ...
    /**
     * @notice Calculate the sample Variance using the existing tuple (count, mean, m2)
     * @param count is the length of the dataset
     * @param m2 is the sum of square errors
     */
    function sampleVariance(uint256 count, uint256 m2)
        internal
        pure
        returns (uint256)
    {
        require(
            count > 1,
            "Count must be larger than 1: sample size must be larger than 2"
        );
    ...
    /**
     * @notice Calculate the population Variance using the existing tuple (count, mean, m2)
     * @param count is the length of the dataset
     * @param m2 is the sum of square errors
     */
    function populationVariance(uint256 count, uint256 m2)
        internal
        pure
        returns (uint256)
    {
    ...
    function stdev(uint256 count, uint256 m2) internal pure returns (uint256) {
        return Math.sqrt(sampleVariance(count, m2));
    }
}
